	...
	class Category
{
    ...
    //! vrátí vrati podkategorii s indexem index
    Category<T>& getSubCategory(unsigned int index) throw(ObjectNotFound);
...
template<class T>
	Category<T>& Category<T>::getSubCategory(Category& cat)
...
template<class T>
	Category<T>& Category<T>::getSubCategory(unsigned int index)
	throw(ObjectNotFound)
{
