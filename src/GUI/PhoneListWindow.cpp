...
{
    PhoneLink* newFile = new PhoneLink("oddělení", "jméno");
    FileEditDialog* editDialog = new FileEditDialog(newFile);
    editDialog->exec();

    db->insertDataFile(*newFile);
    fillTable();
}

void PhoneListWindow::editFile(int index)
{
    if(index == -1) return; // není vybrána položka k editaci
...
void PhoneListWindow::addToTree(int index)
{
    bool ok;
    QString position =
	    QInputDialog::getText(this, tr("Zařazení položky do stromu"),
    ...
    {
	QMessageBox::warning(this, tr("Nekritická chyba"),
			     tr("Vámi zadaná pozice ve stromu není validní"));
    }
}
